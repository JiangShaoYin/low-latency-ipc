...
void record(const Message *m) {
  ...
  // printf("%ld %ld %ld\n", m->t, t, (t - m->t) / 2);
  static set<time_t> recv_msg_ts_set;
  
  assert(recv_msg_ts_set.find(m->t) == recv_msg_ts_set.end());  // 每个时间戳对应的消息只应被记录一次
  recv_msg_ts_set.insert(m->t);
...
bool next_message() {
  ...
  if (delays.size() == test_case_count) {
    ...
    printf(
        "n=%zu minimum=%.0fns median=%.0fns mean=%.0fns std=%.0fns 95%%=%.0fns 99%%=%.0fns "
        "maximum=%.0fns\n",
        delays.size(), (double)delays[0], median, mean, std, p95, p99, maximum);
    exit(0);
  }

  // 检查下一条消息是否已经到时间
  if (test_cases.empty()) return NULL; // 发送结束
  auto msg_info = test_cases.front();
  if (msg_info.first > now()) return NULL; // 距离第1次开始生成msg的时间 < 当前要发送消息的时间， 还没到发送时间， 
  MessageWithSpinloc *msg_with_spinlock = shmQue.getBack(); // 8192

  if (not msg_with_spinlock) return false;

  Message *m = msg_with_spinlock->getMsg();

  test_cases.pop_front();
  m->t = msg_info.first;
  m->size = msg_info.second;
  ...
  m->checksum = crc32(m);
  msg_with_spinlock->setLockVal(1);
  return true;
}
...
void coreActionAlice(MessageWithSpinloc* msg) {
...
int main() {
  MessageWithSpinloc *msg = nullptr;
  bindCPU(1);
  while (true) {
    next_message();
    msg = shmQue.getFront();
    if (msg) coreActionAlice(msg);
  }
  return 0;
}
